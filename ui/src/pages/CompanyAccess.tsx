...
function usePermissionLabels(): Record<PermissionKey, string> {
  const { t } = useTranslation();
  return {
    "agents:create": t("companyAccess.permissions.agentsCreate"),
    "users:invite": t("companyAccess.permissions.usersInvite"),
    "users:manage_permissions": t("companyAccess.permissions.usersManagePermissions"),
    "tasks:assign": t("companyAccess.permissions.tasksAssign"),
    "tasks:assign_scope": t("companyAccess.permissions.tasksAssignScope"),
    "tasks:manage_active_checkouts": t("companyAccess.permissions.tasksManageActiveCheckouts"),
    "joins:approve": t("companyAccess.permissions.joinsApprove"),
    "environments:manage": t("companyAccess.permissions.environmentsManage"),
  };
}
...
function formatGrantSummary(member: CompanyMember, permissionLabels: Record<PermissionKey, string>, noGrantsLabel: string) {
...
export function CompanyAccess() {
  ...
  const updateMemberMutation = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toasts.failedToUpdateMember"),
        body: error instanceof Error ? error.message : t("common.failed"),
        tone: "error",
      });
    },
  });
  ...
  const approveJoinRequestMutation = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toasts.failedToApproveJoinRequest"),
        body: error instanceof Error ? error.message : t("common.failed"),
        tone: "error",
      });
    },
  ...
  const rejectJoinRequestMutation = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toasts.failedToRejectJoinRequest"),
        body: error instanceof Error ? error.message : t("common.failed"),
        tone: "error",
      });
    },
  ...
  const archiveMemberMutation = useMutation({
        ...
        body:
          result.reassignedIssueCount > 0
            ? result.reassignedIssueCount === 1
              ? t("companyAccess.toasts.issuesCleaned", { count: result.reassignedIssueCount })
              : t("companyAccess.toasts.issuesCleanedPlural", { count: result.reassignedIssueCount })
            : undefined,
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toasts.failedToRemoveMember"),
        body: error instanceof Error ? error.message : t("common.failed"),
        tone: "error",
      });
    },
    ...
    <div className="max-w-6xl space-y-8">
      ...
      <section className="space-y-4">
        ...
        {access?.canApproveJoinRequests && pendingHumanJoinRequests.length > 0 ? (
          <div className="space-y-3 rounded-xl border border-border px-4 py-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <h3 className="text-sm font-semibold">{t("companyAccess.humans.pendingJoins")}</h3>
                <p className="text-sm text-muted-foreground">
                  {t("companyAccess.humans.pendingJoinsDescription")}
                </p>
              </div>
              <Badge variant="outline">{t("companyAccess.humans.pendingCount", { count: pendingHumanJoinRequests.length })}</Badge>
            </div>
            <div className="space-y-3">
              {pendingHumanJoinRequests.map((request) => (
                <PendingJoinRequestCard
                  ...
                  title={
                    ...
                    t("companyAccess.humans.unknownRequester")
                  }
                  subtitle={
                    request.requesterUser?.email ||
                    request.requestEmailSnapshot ||
                    request.requestingUserId ||
                    t("companyAccess.humans.noEmailAvailable")
                  }
                  context={
                    request.invite
                      ? `${request.invite.allowedJoinTypes} join invite${request.invite.humanRole ? ` • default role ${request.invite.humanRole}` : ""}`
                      : t("companyAccess.humans.inviteMetadataUnavailable")
                  }
                  detail={t("companyAccess.humans.submitted", { date: new Date(request.createdAt).toLocaleString() })}
                  approveLabel={t("companyAccess.humans.approveHuman")}
                  rejectLabel={t("companyAccess.humans.rejectHuman")}
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          ...
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">{t("companyAccess.table.noMemberships")}</div>
          ) : (
            members.map((member) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 text-sm text-muted-foreground">
                    {formatGrantSummary(member, permissionLabels, t("companyAccess.permissions.noExplicitGrants"))}
                  </div>
                  <div className="space-y-1 text-right">
                    <div className="flex justify-end gap-2">
                      <Button size="sm" variant="outline" onClick={() => setEditingMemberId(member.id)}>
                        {t("companyAccess.table.edit")}
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
            ...
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2 text-sm">
                  ...
                  >
                    <option value="">{t("companyAccess.table.unset")}</option>
                ...
                <label className="space-y-2 text-sm">
                  ...
                  >
                    <option value="active">{t("companyAccess.editMember.statusActive")}</option>
                    <option value="pending">{t("companyAccess.editMember.statusPending")}</option>
                    <option value="suspended">{t("companyAccess.editMember.statusSuspended")}</option>
                  </select>
                </label>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-lg border border-border px-3 py-3">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    {draftRole
                      ? t("companyAccess.editMember.implicitGrantsWithRole", { role: HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[draftRole] })
                      : t("companyAccess.editMember.implicitGrantsNoRole")}
                  </p>
                      ...
                      <span className="space-y-1">
                        ...
                        {implicitGrantSet.has(permissionKey) ? (
                          <span className="block text-xs text-muted-foreground">
                            {t("companyAccess.editMember.includedByRole", { role: draftRole ? HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[draftRole] : t("companyAccess.table.unset") })}
                          </span>
                        ) : null}
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  {assignedIssuesQuery.isLoading
                    ...
                    : assignedIssues.length === 1
                      ...
                      : t("companyAccess.removeMember.openIssuesPlural", { count: assignedIssues.length })}
                </div>
              </div>

              {assignedIssues.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-sm font-medium">{t("companyAccess.removeMember.reassignmentTitle")}</div>
                  <select
                    ...
                    {assignedIssues.length > 6 ? (
                      <div className="px-3 py-2 text-sm text-muted-foreground">
                        {assignedIssues.length - 6 === 1
                          ...
                          : t("companyAccess.removeMember.moreIssuesPlural", { count: assignedIssues.length - 6 })}
                      </div>
                    ) : null}
                  </div>
                </div>
