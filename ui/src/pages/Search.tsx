...
export function Search() {
  ...
  const scopeLabels: Record<CompanySearchScope, string> = useMemo(() => ({
  ...
  const subgroupLabels: Record<SubGroupKey, string> = useMemo(() => ({
    issues: t("search.scopeIssues"),
    comments: t("search.scopeComments"),
    documents: t("search.scopeDocuments"),
    agents: t("search.scopeAgents"),
...
interface SearchTabContentProps {
  ...
  scopeLabels: Record<CompanySearchScope, string>;
  subgroupLabels: Record<SubGroupKey, string>;
}
...
function SearchTabContent({
  ...
  if (showInitialState) {
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        <div>
          <h2 className="text-lg font-semibold">{t("search.initialState.heading")}</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("search.initialState.description")}
          </p>
        </div>
        {recentSearches.length > 0 ? (
          <div>
            <div className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
              {t("search.initialState.recentSearches")}
            </div>
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            <span className="font-medium text-foreground">{t("search.initialState.tipIdentifier")}</span>{" "}
            {t("search.initialState.tipIdentifierDesc", { example: "PAP-123" })}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("search.initialState.tipQuotes")}</span>{" "}
            {t("search.initialState.tipQuotesDesc")}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("search.initialState.tipCmdK")}</span>{" "}
            {t("search.initialState.tipCmdKDesc")}
          </li>
        </ul>
      </div>
    );
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {status ? t("search.error.serverError", { status }) : t("search.error.requestFailed")} {t("search.error.description")}
        </p>
    ...
    return (
      <div className="flex flex-col gap-2 px-2 py-3 sm:px-4">
        <div className="px-3 text-xs text-muted-foreground" data-testid="search-loading">
          {t("search.loading.searching", { query: trimmedQuery })}
        </div>
  ...
  if (isEmpty) {
    const scopeDescription = scope === "all" ? t("search.scopeAllScopes").toLowerCase() : scopeLabels[scope].toLowerCase();
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {t("search.empty.description", { scope: scopeDescription })}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
        ...
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          ...
          <li>
            {t("search.empty.tip2", { example: "PAP-123" })}
          </li>
  ...
  return (
    <div className="flex w-full max-w-[960px] flex-col px-2 sm:px-4" data-testid="search-results">
      <div className="flex items-center justify-between py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
        <span>
          {totalResults === 1 ? t("search.results.result") : t("search.results.results", { count: totalResults })} · {t("search.results.sortedByRelevance")}
        </span>
