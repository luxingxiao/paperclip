...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("profileSettings.breadcrumbSettings") },
      { label: t("profileSettings.breadcrumbProfile") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("profileSettings.displayNamePlaceholder");
  }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.errors.failedToUpdate"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("profileSettings.errors.selectCompanyForAvatar"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.errors.failedToUpload"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("profileSettings.errors.failedToRemove"));
    },
  });
  ...
  const currentName = name.trim() || sessionQuery.data.user.name || t("profileSettings.displayNamePlaceholder");
  ...
  const uploadHint = selectedCompany
    ? t("profileSettings.uploadHint", { company: selectedCompany.name })
    : t("profileSettings.selectCompanyHint");
        ...
        >
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("profileSettings.saving") : t("profileSettings.save")}
            </Button>
          </div>
        </form>
      </section>
