...
export function InstanceAccess() {
  ...
  if (usersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instanceAccess.loading")}</div>;
  }

  if (usersQuery.error) {
    const message =
      usersQuery.error instanceof ApiError && usersQuery.error.status === 403
        ? t("instanceAccess.noPermission")
        ...
        <section className="space-y-4 rounded-xl border border-border bg-card p-5">
          {!selectedUserId ? (
            ...
            <>
              ...
              <div className="space-y-3">
                <div>
                  <h2 className="text-sm font-semibold">{t("instanceAccess.companyAccess.title")}</h2>
                  <p className="text-sm text-muted-foreground">
                    {t("instanceAccess.companyAccess.description")}
                  </p>
                </div>
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {updateCompanyAccessMutation.isPending ? t("instanceAccess.companyAccess.saving") : t("instanceAccess.companyAccess.save")}
                  </Button>
                </div>
              </div>

              <div className="space-y-2">
                <h2 className="text-sm font-semibold">{t("instanceAccess.memberships.title")}</h2>
                <div className="space-y-2">
                    ...
                    >
                      <div>
                        ...
                        <div className="text-muted-foreground">
                          {membership.membershipRole || t("instanceAccess.memberships.unset")} • {membership.status}
                        </div>
                      </div>
