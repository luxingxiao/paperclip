...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? t("approvals.approvalLabel") },
    ]);
  ...
  return (
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("approvals.budgetNotePrefix")} <Link to="/costs" className="underline underline-offset-2">/costs</Link>.
            </p>
          )}
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t("approvals.commentsHeading", { count: comments?.length ?? 0 })}</h3>
