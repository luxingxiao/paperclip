...
export function InstanceExperimentalSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("instanceSettings.experimentalBreadcrumb") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instanceSettings.failedUpdateExperimental"));
    },
  });
    ...
    return (
      <div className="text-sm text-destructive">
        {experimentalQuery.error instanceof Error
          ...
          : t("instanceSettings.failedExperimental")}
      </div>
    );
  }
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instanceSettings.experimentalTitle")}</h1>
        </div>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("instanceSettings.isolatedWorkspacesLabel")}</h2>
          ...
          <button
            type="button"
            data-slot="toggle"
            aria-label={t("instanceSettings.isolatedWorkspacesAria")}
