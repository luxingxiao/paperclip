...
type IssueDetailT = (key: string, opts?: Record<string, unknown>) => string;
...
const ACTION_KEY_MAP: Record<string, string> = {
  ...
  "issue.comment_added": "issueDetail.actionCommented",
  "issue.attachment_added": "issueDetail.actionAttached",
  "issue.attachment_removed": "issueDetail.actionRemovedAttachment",
  "issue.document_created": "issueDetail.actionCreatedDoc",
  "issue.document_updated": "issueDetail.actionUpdatedDoc",
  "issue.document_deleted": "issueDetail.actionDeletedDoc",
...
function formatAction(action: string, details?: Record<string, unknown> | null, t?: IssueDetailT): string {
  const resolve = (key: string, opts?: Record<string, unknown>) =>
    t ? t(key, opts) : (ACTION_KEY_MAP[action] ?? action.replace(/[._]/g, " "));
  if (action === "issue.updated" && details) {
    ...
    if (details.status !== undefined) {
      ...
      parts.push(
        from && t
          ? t("issueDetail.changedStatus", { from: humanizeValue(from), to: humanizeValue(details.status) })
          : from
    ...
    if (details.priority !== undefined) {
      ...
      parts.push(
        from && t
          ? t("issueDetail.changedPriority", { from: humanizeValue(from), to: humanizeValue(details.priority) })
          : from
    ...
    if (details.assigneeAgentId !== undefined || details.assigneeUserId !== undefined) {
      parts.push(
        details.assigneeAgentId || details.assigneeUserId
          ? resolve("issueDetail.assignedIssue")
          : resolve("issueDetail.unassignedIssue"),
      );
    }
    if (details.title !== undefined) parts.push(resolve("issueDetail.updatedTitle"));
    if (details.description !== undefined) parts.push(resolve("issueDetail.updatedDescription"));
  ...
  if (
    ...
    const label = ACTION_KEY_MAP[action] ? resolve(ACTION_KEY_MAP[action]) : action.replace(/[._]/g, " ");
    return `${label} ${key}${title}`;
  }
  const labelKey = ACTION_KEY_MAP[action];
  if (labelKey) return resolve(labelKey);
...
function ActorIdentity({ evt, agentMap }: { evt: ActivityEvent; agentMap: Map<string, Agent> }) {
  ...
  if (evt.actorType === "user") return <Identity name={t("issueDetail.actorBoard")} size="sm" />;
      ...
      <Tabs value={detailTab} onValueChange={setDetailTab} className="space-y-3">
        <TabsList variant="line" className="w-full justify-start gap-1">
          <TabsTrigger value="comments" className="gap-1.5">
            ...
            {t("issueDetail.tabComments")}
          </TabsTrigger>
          <TabsTrigger value="subissues" className="gap-1.5">
            <ListTree className="h-3.5 w-3.5" />
            {t("issueDetail.tabSubIssues")}
          </TabsTrigger>
        ...
        <TabsContent value="activity">
          ...
          {!activity || activity.length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("issueDetail.noActivityItems")}</p>
          ) : (
