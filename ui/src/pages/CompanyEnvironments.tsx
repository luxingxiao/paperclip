...
function SupportMark({ supported }: { supported: boolean }) {
  ...
  return supported ? (
    <span className="inline-flex items-center gap-1 text-green-700 dark:text-green-400">
      ...
      {t("companyEnvironments.table.yes")}
    </span>
  ) : (
    <span className="text-muted-foreground">{t("companyEnvironments.table.no")}</span>
  );
}
...
export function CompanyEnvironments() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("companyEnvironments.title") },
    ]);
    ...
    onSuccess: async (environment) => {
      ...
      pushToast({
        title: editingEnvironmentId
          ? t("companyEnvironments.toasts.environmentUpdated")
          : t("companyEnvironments.toasts.environmentCreated"),
        body: t("companyEnvironments.toasts.ready", { name: environment.name }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyEnvironments.toasts.failedToSave"),
        body: error instanceof Error ? error.message : t("companyEnvironments.actions.failedToSave"),
        tone: "error",
      });
    },
  });
  ...
  const environmentProbeMutation = useMutation({
    ...
    onSuccess: (probe, environmentId) => {
      ...
      pushToast({
        title: probe.ok
          ? t("companyEnvironments.toasts.probePassed")
          : t("companyEnvironments.toasts.probeFailed"),
        body: probe.summary,
    ...
    onError: (error, environmentId) => {
      ...
      setProbeResults((current) => ({
        ...
        [environmentId]: {
          ...
          summary: error instanceof Error ? error.message : t("companyEnvironments.toasts.probeFailed"),
          details: null,
        },
      }));
      pushToast({
        title: t("companyEnvironments.toasts.probeFailed"),
        body: error instanceof Error ? error.message : t("companyEnvironments.toasts.probeFailed"),
        tone: "error",
      });
    },
  });
  ...
  const draftEnvironmentProbeMutation = useMutation({
    ...
    onSuccess: (probe) => {
      pushToast({
        title: probe.ok
          ? t("companyEnvironments.toasts.draftProbePassed")
          : t("companyEnvironments.toasts.draftProbeFailed"),
        body: probe.summary,
        tone: probe.ok ? "success" : "error",
      });
    },
    ...
    return (
      <div className="max-w-3xl space-y-4">
        ...
        <div className="rounded-md border border-border px-4 py-4 text-sm text-muted-foreground">
          {t("companyEnvironments.enableRequired")}
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6" data-testid="company-settings-environments-section">
      ...
      <div className="space-y-4 rounded-md border border-border px-4 py-4">
        <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("companyEnvironments.supportNote")}
        </div>

        <div className="overflow-x-auto">
          <table className="w-full min-w-[34rem] text-left text-xs">
            <caption className="sr-only">{t("companyEnvironments.table.caption")}</caption>
            <thead className="border-b border-border text-muted-foreground">
              <tr>
                <th className="py-2 pr-3 font-medium">{t("companyEnvironments.table.adapter")}</th>
                <th className="px-3 py-2 font-medium">{t("companyEnvironments.table.local")}</th>
                <th className="px-3 py-2 font-medium">{t("companyEnvironments.table.ssh")}</th>
                {sandboxSupportVisible ? (
                  <th className="px-3 py-2 font-medium">{t("companyEnvironments.table.sandbox")}</th>
                ) : null}
              </tr>
            </thead>
        ...
        <div className="border-t border-border/60 pt-4">
          ...
          <div className="space-y-3">
            <Field label={t("companyEnvironments.form.name")} hint={t("companyEnvironments.form.nameHint")}>
              <input
            ...
            <Field label={t("companyEnvironments.form.description")} hint={t("companyEnvironments.form.descriptionHint")}>
              <input
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                type="text"
            ...
            <Field label={t("companyEnvironments.form.driver")} hint={t("companyEnvironments.form.driverHint")}>
              <select
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              ...
              >
                <option value="ssh">{t("companyEnvironments.form.driverSsh")}</option>
                {sandboxCreationEnabled || environmentForm.driver === "sandbox" ? (
                  <option value="sandbox">{t("companyEnvironments.form.driverSandbox")}</option>
                ) : null}
                <option value="local">{t("companyEnvironments.form.driverLocal")}</option>
              </select>
            </Field>

            {environmentForm.driver === "ssh" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label={t("companyEnvironments.form.host")} hint={t("companyEnvironments.form.hostHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("companyEnvironments.form.port")} hint={t("companyEnvironments.form.portHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="number"
                    min={1}
                ...
                <Field label={t("companyEnvironments.form.username")} hint={t("companyEnvironments.form.usernameHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("companyEnvironments.form.remoteWorkspacePath")} hint={t("companyEnvironments.form.remoteWorkspacePathHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                    placeholder={t("companyEnvironments.form.remoteWorkspacePathPlaceholder")}
                    value={environmentForm.sshRemoteWorkspacePath}
                    onChange={(e) =>
                      setEnvironmentForm((current) => ({ ...current, sshRemoteWorkspacePath: e.target.value }))}
                  />
                </Field>
                <Field label={t("companyEnvironments.form.privateKey")} hint={t("companyEnvironments.form.privateKeyHint")}>
                  <div className="space-y-2">
                    <select
                      className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                ...
                <Field label={t("companyEnvironments.form.knownHosts")} hint={t("companyEnvironments.form.knownHostsHint")}>
                  <textarea
                    className="h-32 w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-xs font-mono outline-none"
                ...
                <div className="md:col-span-2">
                  <ToggleField
                    label={t("companyEnvironments.form.strictHostKeyChecking")}
                    hint={t("companyEnvironments.form.strictHostKeyCheckingHint")}
            ...
            {environmentForm.driver === "sandbox" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label={t("companyEnvironments.form.provider")} hint={t("companyEnvironments.form.providerHint")}>
                  <select
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                ...
                <div className="md:col-span-2 space-y-3">
                  ...
                  {selectedSandboxSchema ? (
                    ...
                    <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
                      {t("companyEnvironments.providerNoConfig")}
                    </div>
                  )}
                </div>
              </div>
              ...
              >
                {environmentMutation.isPending
                  ? editingEnvironmentId
                    ? t("companyEnvironments.actions.saving")
                    : t("companyEnvironments.actions.creating")
                  : editingEnvironmentId
                    ? t("companyEnvironments.actions.saveEnvironment")
                    : t("companyEnvironments.actions.createEnvironment")}
              </Button>
              {editingEnvironmentId ? (
                <Button
                  size="sm"
                ...
                >
                  {t("companyEnvironments.actions.cancel")}
                </Button>
              ) : null}
              {environmentForm.driver !== "local" ? (
                ...
                >
                  {draftEnvironmentProbeMutation.isPending
                    ? t("companyEnvironments.testing")
                    : t("companyEnvironments.actions.testDraft")}
                </Button>
              ) : null}
              {environmentMutation.isError ? (
                <span className="text-xs text-destructive">
                  {environmentMutation.error instanceof Error
                    ...
                    : t("companyEnvironments.actions.failedToSave")}
                </span>
              ) : null}
