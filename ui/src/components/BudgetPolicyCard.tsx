...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"], t: (k: string) => string) {
  return windowKind === "lifetime" ? t("budget.lifetimeBudget") : t("budget.monthlyBudget");
}
...
export function BudgetPolicyCard({
  ...
  const wl = (k: BudgetPolicySummary["windowKind"]) => windowLabel(k, t);
    ...
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("budget.utilizationPercent", { percent: summary.utilizationPercent }) : t("budget.noCap")}
        </div>
      </div>
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {t("budget.softAlert", { percent: summary.warnPercent })}{summary.paused && summary.pauseReason ? t("budget.pauseSuffix", { reason: summary.pauseReason }) : ""}
        </div>
      </div>
    </div>
  ) : (
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("budget.agentPaused")
          : t("budget.heartbeatsPaused")}
      </div>
    </div>
  ) : null;
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? t("budget.savingBudget") : summary.amount > 0 ? t("budget.updateBudget") : t("budget.setBudget")}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          <div>
            ...
            <div className="mt-2 text-sm text-muted-foreground">{wl(summary.windowKind)}</div>
          </div>
          <div
            className={cn(
          ...
          >
            ...
            {summary.paused ? t("budget.statusPaused") : summary.status === "warning" ? t("budget.statusWarning") : summary.status === "hard_stop" ? t("budget.statusHardStop") : t("budget.statusHealthy")}
          </div>
        </div>
      ...
      <CardHeader className={cn("gap-3", compact ? "px-4 pt-4 pb-2" : "px-5 pt-5 pb-3")}>
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <CardDescription className="mt-1">{wl(summary.windowKind)}</CardDescription>
          </div>
