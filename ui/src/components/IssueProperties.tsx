...
export function IssueProperties({ issue, onUpdate, inline }: IssuePropertiesProps) {
  ...
  const assigneeTrigger = assignee ? (
    ...
    <>
      <User className="h-3.5 w-3.5 text-muted-foreground" />
      <span className="text-sm text-muted-foreground">{t("issueProperties.noAssignee")}</span>
    </>
  );
  ...
  const assigneeContent = (
    <>
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        >
          {t("issueProperties.clearAssignee")}
        </button>
        {currentUserId && (
          <button
            className={cn(
          ...
          <PropertyRow label={t("issueProperties.workspace")}>
            <div className="w-full space-y-2">
              ...
              {currentExecutionWorkspaceSelection === "reuse_existing" && (
                ...
                >
                  <option value="">{t("issueProperties.chooseWorkspace")}</option>
              ...
              {issue.currentExecutionWorkspace && (
                <div className="text-[11px] text-muted-foreground space-y-0.5">
                  <div style={{ overflowWrap: "anywhere" }}>
                    {t("issueProperties.currentWorkspace")}{" "}
                    <Link
        ...
        {(issue.createdByAgentId || issue.createdByUserId) && (
          <PropertyRow label={t("issueProperties.createdBy")}>
            {issue.createdByAgentId ? (
              ...
              <>
                <User className="h-3.5 w-3.5 text-muted-foreground" />
                <span className="text-sm">{creatorUserLabel ?? t("issueProperties.createdBy")}</span>
              </>
            )}
          </PropertyRow>
        )}
