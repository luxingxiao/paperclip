        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              ...
              {" · "}
              {t("billerSpendCard.providers", { count: row.providerCount })}
              {" · "}
              {t("billerSpendCard.models", { count: row.modelCount })}
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        {budgetMonthlyCents > 0 && (
          <QuotaBar
            ...
            rightLabel={t("billerSpendCard.percentOfAllocation", { percent: Math.round(budgetPct) })}
          />
        )}
                  ...
                  <div key={entry.provider} className="flex items-center justify-between gap-2 text-xs">
                    ...
                    <div className="text-right tabular-nums">
                      ...
                      <div className="text-muted-foreground">
                        {t("billerSpendCard.tokensShort", { count: formatTokens(entry.inputTokens + entry.outputTokens) })}
                      </div>
                    </div>
                  </div>
                ))}
