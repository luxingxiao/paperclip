...
function getProjectStatusLabel(status: string, t: ReturnType<typeof useTranslation>["t"]) {
  switch (status) {
    case "backlog":
      return t("projectProperties.statusBacklog");
    case "planned":
      return t("projectProperties.statusPlanned");
    case "in_progress":
      return t("projectProperties.statusInProgress");
    case "completed":
      return t("projectProperties.statusCompleted");
    case "cancelled":
      return t("projectProperties.statusCancelled");
    default:
      return status;
  }
}
          ...
          {linkedGoals.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {linkedGoals.map((goal) => (
                ...
                >
                  <Link to={`/goals/${goal.id}`} className="hover:underline max-w-55 truncate">
                    {goal.title}
                  </Link>
                  {(onUpdate || onFieldUpdate) && (
                    <button
                      ...
                      aria-label={t("newProjectDialog.removeGoalAria", { goal: goal.title })}
                    >
                      <X className="h-3 w-3" />
                    </button>
                  )}
          ...
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-[11px] text-muted-foreground">
                {t("projectProperties.additionalLegacyWorkspaceRecords")}
              </div>
            )}
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {t("projectProperties.executionWorkspacesDescription")}
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    <button
