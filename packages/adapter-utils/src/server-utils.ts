...
// BEGIN PR #800: Inject wake comment body into agent prompt

export const MAX_WAKE_COMMENT_BODY_CHARS = 8_000;

export function normalizeWakeCommentBody(
  value: unknown,
  maxChars = MAX_WAKE_COMMENT_BODY_CHARS,
): string | null {
  if (typeof value !== "string") return null;
  const trimmed = value.trim();
  if (trimmed.length === 0) return null;
  const sliced = trimmed.length > maxChars ? trimmed.slice(0, maxChars) : trimmed;
  // Avoid leaving a dangling high surrogate when slicing.
  if (/[\uD800-\uDBFF]$/.test(sliced)) {
...
export function appendWakeCommentToPrompt(
  prompt: string,
  ...
  const commentBlock = [
    ...
    "</user_comment>",
    "",
    "A user added the above comment to continue the conversation. Address the user's request.",
  ].join("\n");

  return joinPromptSections([commentBlock, prompt]);
}
